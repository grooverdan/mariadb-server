    ...
    if (len != (size_t)-1)
    {
      ...
      if (found)
      {
        ...
        *end= 0;
        ...
        // 1. Calculate length of the extracted OS name
        size_t val_len = end - found;
        
        // 2. Safely move the OS name, leaving 12 bytes of space at the front for the prefix
        memmove(distribution + 12, found, val_len);
        
        // 3. Null-terminate it
        distribution[12 + val_len] = '\0';
        
        // 4. Now safely copy the prefix into the first 12 bytes
