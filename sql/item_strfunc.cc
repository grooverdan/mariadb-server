...
public:
  ...
  {
    unsigned char *tok_array;
    init_alloc_root(PSI_NOT_INSTRUMENTED, &m_mem_root,
                    max_digest_length + 1024, 0, MYF(0));
    thd->m_digest= &digest;
    // TODO OOM
    tok_array= (unsigned char *) alloc_root(&m_mem_root, max_digest_length);
    digest.m_digest_storage.reset(tok_array, max_digest_length);
    thd->lex = &m_lex;
    thd->push_internal_handler(&m_warning_handler);
    lex_start(thd);
    /* CONTEXT_ANALYSIS_ONLY_VIEW preserves query during processing */
    m_lex.context_analysis_only= CONTEXT_ANALYSIS_ONLY_VIEW;
  }
