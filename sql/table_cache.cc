...
struct Table_cache_instance
{
  ...
  */
  ...
  /**
    ...
    contention in the last 10 seconds for more than 2 seconds, we increase
  ...
  */
  void lock_and_check_contention(uint32_t n_instances, uint32_t instance, my_time_t now)
  {
    ...
    if (mysql_mutex_trylock(&LOCK_table_cache))
    {
      ...
      if (now > (t_last + 2) && mutex_waits * 100 >= tc_threshold * (mutex_nowaits + mutex_waits))
      {
        ...
        /**
          add a second so those threads waiting for the expansion don't
          immediately trigger another expansion.
        */
        t_last= now + 1;
        mutex_waits= 0;
        mutex_nowaits= 0;
